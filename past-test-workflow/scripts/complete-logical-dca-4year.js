// Complete Logical DCA Strategy Test - 4 Years Historical Simulation (2021-2025)
...
// Risk Configuration - Dual Portfolio Architecture
const RISK_CONFIG = {
  PORTFOLIO: {
...
function load4YearCompleteHistoricalData() {
  try {
    ...
    console.log(`💰 Loaded ${btcPriceData.length} BTC price records (4 years)`);
    
    return { fearGreedData, btcPriceData };
    
  } catch (error) {
    console.error('❌ Error loading 4-year complete historical data:', error.message);
    throw error;
  }
}
...
function runComplete4YearLogicalDCATest() {
  console.log('🧠 Starting Complete Logical DCA Strategy Test (Dual Portfolio - 4 Years: 2021-2025)...');
  console.log('📋 Architecture: Core Portfolio (90%) + Satellite Portfolio (10%)');
  
  const { fearGreedData, btcPriceData } = load4YearCompleteHistoricalData();
  ...
  const finalResult = {
    strategy: 'Complete Logical DCA (Core 90% + Satellite 10%) - 4 Years',
    testPeriod: {
      start: commonDates[0],
  ...
  fs.writeFileSync(path.join(RESULTS_DIR, 'complete-logical-dca-results-4year.json'), JSON.stringify(finalResult, null, 2));
  
  // Print summary
  console.log('\\n📊 Complete Logical DCA Test Results (Dual Portfolio - 4 Years):');
  ...
  console.log(`💾 Results saved to: complete-logical-dca-results-4year.json`);
...
if (require.main === module) {
  runComplete4YearLogicalDCATest();
}

module.exports = { runComplete4YearLogicalDCATest };